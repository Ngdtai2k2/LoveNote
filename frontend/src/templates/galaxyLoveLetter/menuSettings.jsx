...
export default function MenuSettings({ settings, onUpdate }) {
  ...
  const initialValues = {
    messages: arrayToMultilineText(settings.messages),
    icons: arrayToMultilineText(settings.icons),
    colors: Array.isArray(settings?.colors) ? settings.colors : [],
    images: [],
    musicId: settings.musicId || 2,
  ...
  const onEmojiPicker = (emojiObject) => {
    const updated = emojiInput + emojiObject.emoji;
    setEmojiInput(updated);
    const arr = multilineTextToArray(updated);
    onUpdate('icons', arr);
  };
              ...
              >
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-1 justify-between items-end">
                    ...
                    />
                    {!isMobile && (
                      <IconButton className="mb-2" onClick={() => setOpenMenuIcons(!openMenuIcons)}>
                        😄
                      </IconButton>
                    )}
                  </div>
                  ...
                  <FormCheckbox
                    ...
                    onChange={(e) => {
                      handleChange(e);
                      onUpdate('cropToHeart', e.target.checked);
                    }}
                    className="my-2"
                  />
